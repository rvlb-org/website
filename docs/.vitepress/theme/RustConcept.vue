...
const concepts = [
  ...
  {
    id: 'elf', icon: '📦', color: '#8b5cf6',
    shortTitle: 'ELF & Loader', title: 'تحليل وحقن', code: 'ELF Binaries',
    subtitle: 'صيغة الملفات التنفيذية وكيفية تشغيلها بدون نظام تشغيل',
    group: 'basics',
    description: [
      'عندما تقوم ببناء مشروعك في Rust (أو C)، لا يخرج لك كود سحري، بل يخرج لك ملف بصيغة تسمى <code>ELF</code> (Executable and Linkable Format). هذا هو المعيار القياسي للبرامج في أنظمة لينكس والأنظمة المدمجة.',
      'في الأنظمة العادية، تقوم نواة لينكس بقراءة هذا الملف وتشغيله تلقائياً. لكن في نظام <strong>Hisn-seL4</strong> ذي النواة الدقيقة (Microkernel)، النواة <em>لا تعرف أصلاً ما هو الملف!</em> لذلك كان لزاماً علينا كتابة <strong>ELF Loader</strong> بأنفسنا في حاوية <code>init</code> لفك شفرة هذه الملفات وحقنها في الذاكرة لتشغيلها.'
    ],
    analogy: {
      icon: '🚚',
      title: 'أمثلة من الحياة اليومية:',
      parts: [
        '<p>تخيل أن <strong>ملف ELF</strong> هو عبارة عن <em>حاوية شحن (Shipping Container)</em> مغلقة تأتي من المصنع (المترجم Compiler). هذه الحاوية لا تحتوي على البضاعة فقط (الكود البرمجي)، بل تحتوي أيضاً على <em>بوليصة شحن</em> أو كتالوج (Metadata) يخبرك بأماكن وضع البضاعة: "ضع النص في الرف أ، والمتغيرات في الرف ب".</p>',
        '<p>أما <strong>الـ ELF Loader</strong> فهو <em>فريق العمال (والرافعة)</em> الذين يقرؤون بوليصة الشحن، يفتحون الحاوية، ينقلون الأكواد إلى أماكنها الصحيحة المحددة مسبقاً في مستودع الذاكرة الافتراضية (Virtual Memory)، ثم يضغطون زر التشغيل.</p>'
      ]
    },
    items: [
      { icon: '🧩', title: 'مكتبة xmas-elf', text: 'نستخدم هذه المكتبة (المكتوبة بلغة Rust) داخل حاوية الإقلاع لتحليل وقراءة ترويسات ملفات ELF وفصل النصوص البرمجية (.text) عن البيانات (.data) بشكل آمن.' },
      { icon: '🚀', title: 'الاستقلالية التامة', text: 'بفضل كتابتنا لـ Loader الخاص بنا، أصبح نظامنا مستقلاً تماماً وقادراً على تشغيل أي برنامج من أي لغة طالما أنه تم تجميعه بصيغة ELF متوافقة مع معماريتنا.' }
    ]
  },
  {
